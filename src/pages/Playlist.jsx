...
const Playlist = () => {
  ...
  // Try to find the playlist in both user playlists and mock playlists
  const playlist = userPlaylists.find((p) => p.id === Number(id)) || 
    ...
    <div className="text-white">
      ...
      <div className="flex items-end space-x-6 bg-gradient-to-b from-[#454545] to-[#121212] p-8">
        <img
          ...
          alt={playlist.name}
          className="w-60 h-60 rounded-xl shadow-2xl"
        />
        <div>
      ...
      <div className="px-8">
        {/* Table Header */}
        <div className="grid grid-cols-[16px,4fr,3fr,2fr,16px] gap-4 px-4 py-2 text-gray-400 border-b border-gray-800 text-sm">
          <div>#</div>
          <div>Title</div>
          <div>Album</div>
